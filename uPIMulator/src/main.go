...
func main() {
	...
	if command_line_parser.IsArgSet("help") {
		...
		global.Init(command_line_parser)
		initGob()

		args_filepath := filepath.Join(global.BinDirpath, "args.txt")
		options_filepath := filepath.Join(global.BinDirpath, "options.txt")
...
func InitCommandLineParser() *misc.CommandLineParser {
	...
	command_line_parser.AddOption(misc.STRING, "image_dirpath",
		"/home/via/uPIMulator/golang/uPIMulator/image", "path to the image directory")
	...
	// attention native=1024
	command_line_parser.AddOption(misc.INT, "wordline_size", "8",
	...
	command_line_parser.AddOption(
		...
		"load_local",
		"0",
		"whether to load MRAM data from local",
	)
...
func initGob() {
	gob.Register(&sram.Wram{})
	gob.Register(&encoding.ByteStream{})
}
