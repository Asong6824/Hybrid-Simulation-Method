...
func (this *LinkerScript) AssignAtomic(executable *kernel.Executable) {
	...
	sections := executable.Sections(kernel.ATOMIC)
	for el := sections.Front(); el != nil; el = el.Next() {
		section := el.Key.(*kernel.Section)
...
func (this *LinkerScript) AssignIram(executable *kernel.Executable) {
	...
	sections := executable.Sections(kernel.TEXT)
	for el := sections.Front(); el != nil; el = el.Next() {
		section := el.Key.(*kernel.Section)
...
func (this *LinkerScript) AssignWram(executable *kernel.Executable) {
	...
	sections := executable.Sections(kernel.DATA)
	for el := sections.Front(); el != nil; el = el.Next() {
		section := el.Key.(*kernel.Section)
	...
	sections = executable.Sections(kernel.RODATA)
	for el := sections.Front(); el != nil; el = el.Next() {
		section := el.Key.(*kernel.Section)
		if section.Name() != "" {
			section.SetAddress(cur_address)
	...
	sections = executable.Sections(kernel.BSS)
	for el := sections.Front(); el != nil; el = el.Next() {
		section := el.Key.(*kernel.Section)
		if section.Name() != "" {
			section.SetAddress(cur_address)
...
func (this *LinkerScript) AssignMram(executable *kernel.Executable) {
	...
	sections := executable.Sections(kernel.DEBUG_ABBREV)
	for el := sections.Front(); el != nil; el = el.Next() {
		section := el.Key.(*kernel.Section)
		section.SetAddress(cur_address)
		cur_address += section.Size()
	...
	sections = executable.Sections(kernel.DEBUG_FRAME)
	for el := sections.Front(); el != nil; el = el.Next() {
		section := el.Key.(*kernel.Section)
		section.SetAddress(cur_address)
		cur_address += section.Size()
	...
	sections = executable.Sections(kernel.DEBUG_INFO)
	for el := sections.Front(); el != nil; el = el.Next() {
		section := el.Key.(*kernel.Section)
		section.SetAddress(cur_address)
		cur_address += section.Size()
	...
	sections = executable.Sections(kernel.DEBUG_LINE)
	for el := sections.Front(); el != nil; el = el.Next() {
		section := el.Key.(*kernel.Section)
		section.SetAddress(cur_address)
		cur_address += section.Size()
	...
	sections = executable.Sections(kernel.DEBUG_LOC)
	for el := sections.Front(); el != nil; el = el.Next() {
		section := el.Key.(*kernel.Section)
		section.SetAddress(cur_address)
		cur_address += section.Size()
	...
	sections = executable.Sections(kernel.DEBUG_RANGES)
	for el := sections.Front(); el != nil; el = el.Next() {
		section := el.Key.(*kernel.Section)
		section.SetAddress(cur_address)
		cur_address += section.Size()
	...
	sections = executable.Sections(kernel.DEBUG_STR)
	for el := sections.Front(); el != nil; el = el.Next() {
		section := el.Key.(*kernel.Section)
		section.SetAddress(cur_address)
		cur_address += section.Size()
	...
	sections = executable.Sections(kernel.STACK_SIZES)
	for el := sections.Front(); el != nil; el = el.Next() {
		section := el.Key.(*kernel.Section)
		section.SetAddress(cur_address)
		cur_address += section.Size()
	...
	cur_address = 0x8080000

	// stdio.__stdout_buffer
	...
	sections = executable.Sections(kernel.MRAM)
	for el := sections.Front(); el != nil; el = el.Next() {
		section := el.Key.(*kernel.Section)
